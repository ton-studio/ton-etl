...
def parquet_monthly_converter():
    ...
    def safe_python_callable(func, kwargs, step_name):
        try:
            ...
            send_notification(f"📛 [parquet_monthly_converter] Unable to invoke {func.__name__} for *{step_name}")
            raise e
    ...
    def convert_table(kwargs):
        ...
        month = logical_time.format('YYYYMM')

        logging.info(f"Running conversion for {source_table_name} in {month}")
        ...
        where {partition_field} like '{month}%'
        """
...
parquet_monthly_converter_dag = parquet_monthly_converter()
